...
{
public:
  void notifyDeletedThread(Thread * deleted){
    deleted->_synchronizer = 0;
    queue.remove(&deleted->_link);
  }
protected:
    Synchronizer_Common() {}
    ~Synchronizer_Common() { wakeup_all(); }

    void operator=(Synchronizer_Common * s){}

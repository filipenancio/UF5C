...
template<> struct Traits<Serial_Display>: public Traits<void>
{
    static const bool enabled = true;
    enum {UART, USB};
    static const int ENGINE = Traits<Build>::MODEL == Traits<Build>::eMote3 ? USB : UART;
...
template<> struct Traits<Network>: public Traits<void>
{
    ...
    typedef IF<Traits<Build>::MODEL == Traits<Build>::eMote3, TSTPOTM, TSTPOE>::Result TSTPMAC;

    // This list is positional, with one network for each NIC in Traits<NIC>::NICS
    typedef LIST<TSTPMAC> NETWORKS;
};

template<> struct Traits<TSTP>: public Traits<Network>
{
    typedef Traits<Network>::TSTPMAC MAC;
    ...
    static const unsigned int Security = TSTP_Security;

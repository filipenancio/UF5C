...
public:
    class Node{
        public:
            ...
            Node(const ID & id, const Coordinates & coor): _id(id), _coor(coor){
                auth(id, & _auth);
            }

            Node(const Coordinates &coor): _coor(coor){}

            void id(const ID & id){ _id = id; auth(id, &_auth);}
            ID id() const {return _id;}

            Auth auth() const {return _auth;}

            void coordinates(const Coordinates &coor){ _coor = coor;}
            const Coordinates coordinates() const { return _coor; }

            Diffie_Hellman::Public_Key public_key(){return _dh.public_key();}
            
            void calc_key(Diffie_Hellman::Public_Key k){
                _master_secret = _dh.shared_key(k);
            }

            Diffie_Hellman::Shared_Key secret(){return _master_secret;}
...
public:
    TSTP_Security() : _sem(0) {
        ...
        const char sensor_group_id[16] = {0,0,0,0,0,0,0,1,10,10,1,2,3,3,2,1};
        ID id_temp((unsigned char *)sensor_group_id);
        _id_group = id_temp;
        auth(_id_group, &_auth_group);
    ...
    static void addNode(const char unsigned * id, const Coordinates & coordinates){ _instance->insertListNode(Node(id, coordinates));}
...
private:
    ...
    void insertListNode(Node n){listNode.insert(new Simple_List<Node>::Element(& n));}
    ...
    Auth _auth_group;
    Diffie_Hellman _dh;
    Diffie_Hellman _dh_group;
    
    Cipher _aes;
    Poly1305 _poly;
    
    Diffie_Hellman::Shared_Key _master_secret_id;
    Diffie_Hellman::Shared_Key _master_secret_group;
...
public:
    ...
    class Group_DH_Request: public Control
    {
    public:
        Group_DH_Request(const Coordinates & dst, const Diffie_Hellman::Public_Key & k) 
            : Control(GROUP_DH_REQUEST, 0, 0, here(), here(), 0, 0), _destination(dst), _public_key(k) { }
        ...
        friend Debug & operator<<(Debug & db, const Group_DH_Request & m) {
            db << reinterpret_cast<const Control &>(m) << ",d=" << m._destination << ",k=" << m._public_key;
            return db;
        }
    ...
    class Group_DH_Response: public Control
    {
    public:
        Group_DH_Response(const Diffie_Hellman::Public_Key & k) 
            : Control(GROUP_DH_RESPONSE, 0, 0, here(), here(), 0, 0), _public_key(k) { }

        Diffie_Hellman::Public_Key key() { return _public_key; } 
        void key(const Diffie_Hellman::Public_Key & k) { _public_key = k; } 

        friend Debug & operator<<(Debug & db, const Group_DH_Response & m) {
            db << reinterpret_cast<const Control &>(m) << ",k=" << m._public_key;
            return db;
        }
    ...
    class Group_Auth_Request: public Control
    {
    public:
        Group_Auth_Request(const Auth & a, const OTP & o) 
            : Control(GROUP_AUTH_REQUEST, 0, 0, here(), here(), 0, 0), _auth(a), _otp(o) { }
        ...
        friend Debug & operator<<(Debug & db, const Group_Auth_Request & m) {
            db << reinterpret_cast<const Control &>(m) << ",a=" << m._auth << ",o=" << m._otp;
            return db;
        }
    ...
    class Group_Auth_Granted: public Control
    {
    public:
        Group_Auth_Granted(const Coordinates & dst, const Auth & a) 
            : Control(GROUP_AUTH_GRANTED, 0, 0, here(), here(), 0, 0), _destination(dst), _auth(a) { }
        ...
        friend Debug & operator<<(Debug & db, const Group_Auth_Granted & m) {
            db << reinterpret_cast<const Control &>(m) << ",d=" << m._destination << ",a=" << m._auth;
            return db;
        }
...
public:
    ...
    static void addNode(const char unsigned * id, const Coordinates & coordinates) {
        Security::addNode(id, coordinates);
    }
private:
